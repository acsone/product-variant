...
class TestProductPrice(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.product_red_s = cls.template.product_variant_ids.filtered(
            lambda p: cls.attribute_color_red.id in 
                p.product_template_attribute_value_ids.product_attribute_value_id.ids 
                and cls.attribute_size_s.id in 
                p.product_template_attribute_value_ids.product_attribute_value_id.ids)
        ...
        cls.product_red_m = cls.template.product_variant_ids.filtered(
            lambda p: cls.attribute_color_red.id in 
            p.product_template_attribute_value_ids.product_attribute_value_id.ids 
            and cls.attribute_size_m.id in 
            p.product_template_attribute_value_ids.product_attribute_value_id.ids)
        cls.pricelist_base.write({
            "item_ids": [Command.create({
    ...
    def test_price(self):
        self.assertEqual(
            6.0,
            self.product_red_s.pricelist_base_price
        )
        self.assertEqual(
            5.0,
            self.product_red_m.pricelist_base_price
        )
        self.assertEqual(
            self.pricelist_base,
            self.product_red_m.base_pricelist_id
        )